...
def average_polynomial_curve():
    ...
    for sound, group in grouped:
        ...
        SUM_PLOT.plot(time_points, avg_curve, label=sound)
        SUM_PLOT.fill_between(time_points, avg_curve - std_dev, avg_curve + std_dev, alpha=0.2)
    
    SUM_PLOT.legend()
    SUM_PLOT.set_title('Average Polynomial Curves for Each Sound')
...
def isStable():
    ...
    if len(ANGLE_HISTORY) == ANGLE_HISTORY.maxlen:
        # Calculate the standard deviation of angles
        angle_std = np.std(ANGLE_HISTORY)

        # Determine stability
        if angle_std >= PARAMS["stab_thresh"]:
            return False
